...
import dolphin.model as model_config
...
from dolphin.model.doc_analyze_by_custom_model import (batch_doc_analyze,
                                                         doc_analyze)
...
def _do_parse(
    ...
    layout_model=None,
    formula_enable=None,
    table_enable=None,alg_method_type='mineru', num_parrlel_inference=200,
        figure_vlm=False
):
            ...
            if parse_method == 'auto':
                if ds.classify() == SupportedPdfParseMethod.TXT:
                    infer_result = ds.apply(
                        ...
                        table_enable=table_enable,alg_method_type=alg_method_type, num_parrlel_inference=num_parrlel_inference,
...
def do_parse(
    ...
    parallel_count = 1
    # parallel_count = num_parrlel_inference
    ...
    if parallel_count > 1:
        if isinstance(pdf_bytes_or_dataset, bytes):
            ...
            ds = pdf_bytes_or_dataset
        batch_do_parse(output_dir, [pdf_file_name], [ds], parse_method, debug_able, f_draw_span_bbox=f_draw_span_bbox, f_draw_layout_bbox=f_draw_layout_bbox, f_dump_md=f_dump_md, f_dump_middle_json=f_dump_middle_json, f_dump_model_json=f_dump_model_json, f_dump_orig_pdf=f_dump_orig_pdf, f_dump_content_list=f_dump_content_list, f_make_md_mode=f_make_md_mode, f_draw_model_bbox=f_draw_model_bbox, f_draw_line_sort_bbox=f_draw_line_sort_bbox, f_draw_char_bbox=f_draw_char_bbox, lang=lang,alg_method_type=alg_method_type, num_parrlel_inference=num_parrlel_inference,
        figure_vlm=figure_vlm)
    else:
        _do_parse(output_dir, pdf_file_name, pdf_bytes_or_dataset, model_list, parse_method, debug_able, start_page_id=start_page_id, end_page_id=end_page_id, lang=lang, layout_model=layout_model, formula_enable=formula_enable, table_enable=table_enable,  f_draw_span_bbox=f_draw_span_bbox, f_draw_layout_bbox=f_draw_layout_bbox, f_dump_md=f_dump_md, f_dump_middle_json=f_dump_middle_json, f_dump_model_json=f_dump_model_json, f_dump_orig_pdf=f_dump_orig_pdf, f_dump_content_list=f_dump_content_list, f_make_md_mode=f_make_md_mode, f_draw_model_bbox=f_draw_model_bbox, f_draw_line_sort_bbox=f_draw_line_sort_bbox, f_draw_char_bbox=f_draw_char_bbox,alg_method_type=alg_method_type, num_parrlel_inference=num_parrlel_inference,
        figure_vlm=figure_vlm)
...
def batch_do_parse(
    ...
    infer_results = batch_doc_analyze(dss, parse_method, lang=lang, layout_model=layout_model, formula_enable=formula_enable, table_enable=table_enable,alg_method_type=alg_method_type, num_parrlel_inference=num_parrlel_inference)
...
parse_alg_methods = click.Choice(['mineru', 'dolphin'])