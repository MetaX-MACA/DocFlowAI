...
def get_figure_vlm_config():
    config = read_config()
    t_config = config.get('figure_vlm-config')
    models_dir_Qwen25_VL_3B = config.get('models-dir_Qwen25_VL_3B')
    t_config['models_dir_Qwen25_VL_3B']=models_dir_Qwen25_VL_3B
    if t_config is None:
        logger.warning(f"'t-config' not found in {CONFIG_FILE_NAME}, use 'False' as default")
        return json.loads(f'{{"model": "{MODEL_NAME.RAPID_TABLE}","enable": false, "max_time": 400}}')
    else:
        return t_config
def get_dolphin_config():
    config = read_config()
    t_config = config.get('Dolphin-config')
    models_dir_Dolphin = config.get('models-dir_Dolphin')
    t_config['models_dir_Dolphin']=models_dir_Dolphin
    if t_config is None:
        logger.warning(f"'t-config' not found in {CONFIG_FILE_NAME}, use 'False' as default")
        return json.loads(f'{{"model": "{MODEL_NAME.RAPID_TABLE}","enable": false, "max_time": 400}}')
    else:
