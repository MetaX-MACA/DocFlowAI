...
from dolphin.model.dolphin.demo_page_hf import DOLPHIN
...
class CustomPEKModel:
    ...
    def __init__(self, ocr: bool = False, show_log: bool = False, **kwargs):
        ...
        figure_vlm_config=kwargs.get('figure_vlm_config',)
        dolphin_config=kwargs.get('dolphin_config')
        alg_method_type=kwargs.get('alg_method_type')

        if alg_method_type == "dolphin":
            model = DOLPHIN(dolphin_config['models_dir_Dolphin'])
            self.dolphin_model = model
            pass

        if figure_vlm_config and figure_vlm_config.get('enable'):
            # https://hf-mirror.com/Qwen/Qwen2.5-VL-3B-Instruct
            print(f"figure_vlm_config: {figure_vlm_config}")
            from transformers import Qwen2_5_VLForConditionalGeneration, AutoTokenizer, AutoProcessor
            # from qwen_vl_utils import process_vision_info

            # default: Load the model on the available device(s)
            model = Qwen2_5_VLForConditionalGeneration.from_pretrained(figure_vlm_config['models_dir_Qwen25_VL_3B'], torch_dtype="auto", device_map="auto"
            )
            # default processer
            processor = AutoProcessor.from_pretrained(figure_vlm_config['models_dir_Qwen25_VL_3B'])
