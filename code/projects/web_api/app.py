...
from dolphin.model.doc_analyze_by_custom_model import doc_analyze as do_parse_dolphin
...
def process_file(
    ...
    file_extension: str,
    parse_method: str,
    image_writer: Union[S3DataWriter, FileBasedDataWriter],method_type,figure_vlm
    ...
    # , method_type, figure_vlm

    if method_type=='dolphin':

        if parse_method == "ocr":
            infer_result = ds.apply(do_parse_dolphin, ocr=True, alg_method_type=method_type, figure_vlm=figure_vlm)
            pipe_result = infer_result.pipe_ocr_mode(image_writer)
        elif parse_method == "txt":
            infer_result = ds.apply(do_parse_dolphin, ocr=False, alg_method_type=method_type, figure_vlm=figure_vlm)
...
async def file_parse(
    ...
    method_type: str = Form("mineru"),
    figure_vlm: bool = Form(False),
):
    """
    ...
    try:
        ...
        # Process PDF
        infer_result, pipe_result = process_file(file_bytes, file_extension, parse_method, image_writer,method_type,figure_vlm)
